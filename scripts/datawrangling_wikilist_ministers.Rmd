---
title: "data_wrangling_list_ministers_wiki"
author: "Stefan Haußner"
date: "11 3 2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(dplyr)
library(readr)
library(tidyr)
library(purrr)
library(stringr)
library(countrycode)
library(fuzzyjoin)
library(janitor)
library(stringdist)
```

```{r warning=FALSE, message=FALSE}
ministers <- read_csv2("../data/list_scraped_gov_with_party.csv")
parties <- read_csv("../data/list_european_parties.csv")


```

# Prepare to join

Zusatzfrage: Wie gehen wir mit "nominated by" etc. um?

Problem: In Minister-Liste sind sehr unterschiedliche Parteibezeichner (meist Abkürzung oder englischsprachige Vollnamen)

```{r}
ministers <- ministers %>%
  mutate(cntry = countrycode(cntry, "iso2c","iso3c")) %>% 
  group_by(cntry) %>% 
  mutate(id = row_number())

parties <- parties %>% 
  group_by(iso) %>% 
  mutate(id = row_number())
```

# Functions

## Suche der Matchposition

pm = perfect match
gm = grepl match
fm = fuzzy match

```{r}
f_findmatchposition_gm <- function(ministerparty, partylist){
  gm_abbr <- grepl(pattern = ministerparty, x = partylist$abbr)
  gm_name <- grepl(pattern = ministerparty, x = partylist$name)
  gm_nameen <- grepl(pattern = ministerparty, x = partylist$name_en)
  gm_abbr2 <- str_detect(string = ministerparty, pattern = partylist$abbr2)
  
  gm <- tibble(gm_abbr = gm_abbr, gm_name = gm_name, gm_nameen = gm_nameen) %>% 
    rowwise() %>% 
    mutate(any = case_when(gm_abbr == TRUE ~ TRUE,
                           gm_name == TRUE ~ TRUE,
                           gm_nameen == TRUE ~ TRUE,
                           TRUE ~ FALSE)) %>% 
    pull(any)
  if (any(gm)) {position <- which(gm)[1]} else {position <- NA_integer_}

  return(position)
  }
```


```{r}
f_findmatchposition_pm <- function(ministerparty, partylist){
  
  pm_abbr <- partylist$abbr %in% ministerparty
  pm_name <- partylist$name %in% ministerparty
  pm_nameen <- partylist$name_en %in% ministerparty
  
  pm <- tibble(pm_abbr = pm_abbr, pm_name = pm_name, pm_nameen = pm_nameen) %>% 
    rowwise() %>% 
    mutate(any = case_when(pm_abbr == TRUE ~ TRUE,
                           pm_name == TRUE ~ TRUE,
                           pm_nameen == TRUE ~ TRUE,
                           TRUE ~ FALSE)) %>% 
    pull(any)

  if (any(pm)) {position <- which(pm)[1]} else {position <- NA_integer_}

  return(position)
}
```

```{r}
f_findmatchposition_fm <- function(ministerparty, partylist){
  
  ministerparty <- str_remove_all(ministerparty, pattern = stop_pattern)
  
  d_jc <- stringdist(ministerparty, partylist, method = "jaccard")
  d_jw <- stringdist(ministerparty, partylist, method = "jw")
  d_osa <- stringdist(ministerparty, partylist, method = "osa")
  d_lcs <- stringdist(ministerparty, partylist, method = "lcs")
  
  position <- bind_cols(jaccard = d_jc, jw = d_jw, osa = d_osa, lcs = d_lcs) %>% 
    mutate(pos = row_number()) %>% 
    pivot_longer(jaccard:lcs) %>% 
    group_by(name) %>% 
    arrange(name) %>% 
    filter(value == min(value)) %>% 
    ungroup() %>% 
    count(pos) %>% 
    filter(n == max(n)) %>% 
    pull(pos) %>% 
    .[1]

  return(position)
  }
```


## Link position to with ministers

```{r}
f_linkposition <- function(cntry_ministers, cntry_parties){
  m_to_link <- cntry_ministers %>%
    as.data.frame() %>% 
    as_tibble() %>% 
    mutate(party = str_to_lower(party)) %>% 
    select(id, party)
  
  p_to_link <- cntry_parties %>%
    as.data.frame() %>% 
    as_tibble() %>% 
    mutate(name = str_to_lower(name),
           abbr = str_to_lower(abbr),
           abbr2 = str_to_lower(abbr) %>% str_remove_all(pattern = "[^A-Za-z0-9]"),
           name_en = str_to_lower(name_en)) %>% 
    select(id, name, abbr, name_en, abbr2)
  
  ministers_with_pos <- m_to_link %>% 
    mutate(pos = map_int(party, f_findmatchposition_pm, p_to_link),
           pos = case_when(is.na(pos) ~ map_int(party, f_findmatchposition_gm, p_to_link),
                           TRUE ~ pos),
           pos = case_when(is.na(pos) ~ map_int(party, f_findmatchposition_fm, p_to_link),
                           TRUE ~ pos),
           id = row_number()) %>% 
    select(id, pos) %>% 
    left_join(cntry_ministers, ., by = c("id"))
  return(ministers_with_pos)
}
```


```{r}
ministers_nested <- ministers %>% group_by(cntry) %>% nest() %>% rename("ministers" = "data")
parties_nested <- parties %>% group_by(iso) %>% nest() %>% rename("parties" = "data")
nested_joined <- left_join(ministers_nested, parties_nested, by = c("cntry" = "iso"))
```

```{r}
stop_pattern <- ministers$cntry %>% 
  unique() %>% 
  countrycode(origin = "iso3c", destination = "country.name") %>% 
  c(., "[:blank:]*[Tt]he[:blank:]","[:blank:]of[:blank:]","[:blank:]in[:blank:]") %>% 
  str_c(collapse = "|")
```


```{r}
nested_joined %>% pull(cntry)
```

Noch offen:

nominated by etc. -> Partei oder independent?
pure Independents
Achtung bei BGR ist der Join von NA falsch. NA party wird gematcht über abbr == NA. Lösbar über Austausch vorher von NAs zu "unaffiliated" etc.
fuzzy match über distanzen (siehe playground)

```{r}
test <- nested_joined %>%
  mutate(m_with_pos = map(ministers, f_linkposition, parties)) %>% 
  select(cntry, m_with_pos) %>% 
  unnest(m_with_pos) %>% 
  left_join(., parties, by = c("cntry" = "iso", "pos" = "id")) %>% 
  clean_names() %>% 
  select(cntry, 
         name_minister = name_x, 
         portfolio, 
         party, 
         party_fullname = name_y, 
         party_abbr = abbr, 
         party_nameen = name_en, 
         orientation:eu_stance)
 
test 

```

# Playground

 ! Achtung bei BGR ist der Join von NA falsch. NA party wird gematcht über abbrv == NA. Lösbar über Austausch vorher von NAs zu "unaffiliated" etc.
 Bulgarien jetzt noch mehr falsch

```{r}
f_findmatchposition_fm <- function(ministerparty, partylist){
  
  ministerparty <- str_remove_all(ministerparty, pattern = stop_pattern)
  
  d_jc <- stringdist(ministerparty, partylist, method = "jaccard")
  d_jw <- stringdist(ministerparty, partylist, method = "jw")
  d_osa <- stringdist(ministerparty, partylist, method = "osa")
  d_lcs <- stringdist(ministerparty, partylist, method = "lcs")
  
  position <- bind_cols(jaccard = d_jc, jw = d_jw, osa = d_osa, lcs = d_lcs) %>% 
    mutate(pos = row_number()) %>% 
    pivot_longer(jaccard:lcs) %>% 
    group_by(name) %>% 
    arrange(name) %>% 
    filter(value == min(value)) %>% 
    ungroup() %>% 
    count(pos) %>% 
    filter(n == max(n)) %>% 
    pull(pos) %>% 
    .[1]

  return(position)
  }
```

```{r}


test_parties <- parties %>% 
  filter(iso == "FIN") %>%
  pull(name_en)

test_ministers <- ministers %>% 
  filter(cntry == "FIN") %>% 
  mutate(party_stop = str_remove_all(party, pattern = stop_pattern)) %>% 
  select(name, party, party_stop)

d_jc <- stringdist::stringdist(test_ministers$party[4], test_parties, method = "jaccard")
d_jw <- stringdist::stringdist(test_ministers$party[4], test_parties, method = "jw")
d_osa <- stringdist::stringdist(test_ministers$party[4], test_parties, method = "osa")
d_lcs <- stringdist::stringdist(test_ministers$party[4], test_parties, method = "lcs")
bind_cols(jaccard = d_jc, jw = d_jw, osa = d_osa, lcs = d_lcs) %>% 
  mutate(pos = row_number()) %>% 
  pivot_longer(jaccard:lcs) %>% 
  group_by(name) %>% 
  arrange(name) %>% 
  filter(value == min(value)) %>% 
  ungroup() %>% 
  count(pos) %>% 
  filter(n == max(n)) %>% pull(pos)



test_parties
```

