...
ministers_it <- left_join(tm, td, by ="portfolio") %>% mutate(cntry = "IT")
```
...
ministers_se <- read_html(url) %>% 
  ...
  clean_names() %>% 
  filter(minister == "") %>% 
  select(name = minister_1, portfolio, party, took_office, left_office) %>% 
  mutate(cntry = "SE")
...
```{r}
ls(pattern = "ministers_") %>% length() == 27
```
...
```{r}
ministers_all <- mget(ls(pattern="ministers_")) %>%
              bind_rows() %>% 
  arrange(cntry) %>% 
...
write_csv2(ministers_all, file = "../data/list_scraped_gov_with_party.csv")
```
